...
def run_rl_train(
    ...
    if experiment.endswith("empathy_full"):
        num_steps = 2000
    print("="*30)
    ...
    set_seed(seed)
    if learning_mode not in ["bandit", "weighted", "bandit_weighted", "single", "round"]:
    ...
    if learning_mode != "single":
        experiment = "rl_scst_"+ learning_mode +"_"+ experiment
    else:
        experiment = "rl_scst_"+ learning_mode +"_"+  str(single_reward_idx) + "_" + experiment
    if do_wandb:
    ...
    step_count = 0
    cont = True
    if "bandit_weighted" == learning_mode:
    ...
    elif "bandit" == learning_mode: 
        bandit = Exp3(len(scorers))
        bandit_history = []
        bandit_weight_history = []
        bandit_arm_weight_history = []
        chosen = bandit.draw()
        ...
        print("Bandit arm pulled:", chosen)
        rl_scorer_history = { k["name"]+"_scores":[] for k in scorer.scorers }
        bandit_pulls = { i:0 for i in range(len(scorer.scorers)) }
        bandit_pulls[last_chosen] += 1
        bandit_history.append(last_chosen)
        bandit_arm_weight_history.append(bandit.weights.copy())
        print("Bandit Pull:", bandit_pulls)
    ...
    while cont:
        for ib, paragraphs in enumerate(tqdm(dataloader, position=0, leave=True, dynamic_ncols=True)):
            ...
            if step_count !=0 and  step_count % rl_validation_step == 0 and ("bandit" in learning_mode ):
                ...
                bandit_arm_weight_history.append(bandit.weights.copy())
                if "bandit_weighted" in learning_mode:
    ...
    if "bandit" in learning_mode:
        with open(f"{output_dir}/bandit_history.json", "w") as f:
            json.dump(bandit_history, f, indent=4)
        with open(f"{output_dir}/bandit_weights.json", "w") as f:
            json.dump(bandit.weights, f, indent=4)
        with open(f"{output_dir}/scorer_weight_history.json", "w") as f:
            json.dump(bandit_weight_history, f, indent=4)
        with open(f"{output_dir}/bandit_arm_weight_history.json", "w") as f:
            json.dump(bandit_arm_weight_history, f, indent=4)
    if test:
