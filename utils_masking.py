...
def string2mask(masker_name):
    if masker_name[:2] == "cp":
        return ClientPromptKeywordMasker(mask_ratio=int(masker_name[2:])/100.0)
    elif masker_name[:2] == "kw":
...
class KeywordMasker(Masker):
    ...
    def mask(self, sentences):
        ...
        return unmasked, masked, is_masked, self.compute_effective_mask_ratio(is_masked)
class ClientPromptKeywordMasker(KeywordMasker):
    """ 
    NOTE: THis is unsued: i'm doing this with
           a custom CoverageModel (easier there
           since sent_tokenize happens there)
    This is a variant of KeywordMasker.
    It's aim is to make the coverage problem
    only targeted at Client Utterance, so that 
    the score reflects how well keywords (hopefully covering topic, entities)
    are recovoered from Counselor prompt
    """
    def mask(self, sentences):
        """
        Ignoring counselor sentence
        """
        new_sentences = []
        for sentence in sentences:
            print(sentence)
            if len(sentence.split("Counselor:"))==2:
                new_sentences += [ sentence.split("Counselor:")[0] ]
            else:
